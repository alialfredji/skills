...
existing HTML mockup, do not regenerate the frame. Read the current local MDX,
replace a unique snippet surgically, preserve stable identifiers and unrelated
markup, then rerun the local check and renderer-backed verify commands for the
plan directory.
