...
**Patching.** Edit one wireframe, canvas annotation, diagram, or block directly
in the local MDX rather than regenerating the whole plan. `plan.mdx` holds
frontmatter plus document blocks; `canvas.mdx` holds
`<DesignBoard>/<Section>/<Artboard>/<Screen>/<Annotation>/<Connector>`. Read the
complete source before editing, preserve every unrelated block and surface, use
stable identifiers, and rerun local check and renderer-backed verify immediately
after the update.
