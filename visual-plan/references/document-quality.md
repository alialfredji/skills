...
machine-checked list of block types and their data schemas, query the running
local viewer with `local-viewer.sh cli plan blocks --app-url
http://127.0.0.1:8096`. It returns the renderer vocabulary so you never emit a
block the editor cannot render or round-trip:
